...
def parse_content(d:dict):
    ...
    def get_paragraphs(h):
        if h is not None:
            paragraphs = [p.text for p in h.find_all('dl', class_='RspDL') if len(p.text)>0]
        else:
            paragraphs = None
        return paragraphs
    # --------------------------------------------------------------------------------
    ...
    parsed = {get_label(topmore):get_paragraphs(topmore.next_sibling) for topmore in soup.find_all('div', class_='docLayoutMarginTopMore')}
    d.update(parsed)
...
if __name__ == '__main__':
    ...
    populate_es_streaming_bulk(
        es_client, dicts_g, INDEX_NAME, TYPE
    )